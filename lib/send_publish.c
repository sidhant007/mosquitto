...
{
    const struct vec buffer = {payload, payloadlen};
    return send__publishv(mosq, mid, topic, payloadlen, &buffer, 1, qos, retain, dup, cmsg_props, store_props, expiry_interval);
}

int send__publishv(struct mosquitto *mosq, uint16_t mid, const char *topic, uint32_t payloadlen, const struct vec *buffers, int buffers_cnt, int qos, bool retain, bool dup, const mosquitto_property *cmsg_props, const mosquitto_property *store_props, uint32_t expiry_interval)
{
#ifdef WITH_BROKER
	size_t len;
#ifdef WITH_BRIDGE
		...
		for(i=0; i<mosq->bridge->topic_count; i++){
			...
			if((cur_topic->direction == bd_both || cur_topic->direction == bd_out)
				...
				if(match){
					...
					rc =  send__real_publishv(mosq, mid, mapped_topic, payloadlen, buffers, buffers_cnt, qos, retain, dup, cmsg_props, store_props, expiry_interval);
					mosquitto__free(mapped_topic);
					return rc;
				}
			}
	...
	return send__real_publishv(mosq, mid, topic, payloadlen, buffers, buffers_cnt, qos, retain, dup, cmsg_props, store_props, expiry_interval);
}
...
{
    const struct vec buffer = {payload, payloadlen};
    return send__real_publishv(mosq, mid, topic, payloadlen, &buffer, 1, qos, retain, dup, cmsg_props, store_props, expiry_interval);
}

int send__real_publishv(struct mosquitto *mosq, uint16_t mid, const char *topic, uint32_t payloadlen, const struct vec *buffers, int buffers_cnt, int qos, bool retain, bool dup, const mosquitto_property *cmsg_props, const mosquitto_property *store_props, uint32_t expiry_interval)
{
