 ...
 *               so they can be tracked with this parameter.
 *  topic -      null terminated string of the topic to publish to.
 *  buffers -    an array of vec instances, where each instance consists of a 
 ...
 *	<mosquitto_max_inflight_messages_set>
 */
libmosq_EXPORT int mosquitto_publishv(struct mosquitto *mosq, int *mid, const char *topic, const struct vec *buffers, int buffers_cnt, int qos, bool retain);

/*
 * Function: mosquitto_publishv_v5
 *
 * Publish a message on a given topic, with attached MQTT properties.
 *
 * Use e.g. <mosquitto_property_add_string> and similar to create a list of
 ...
 *	                           supported by the broker.
 */
libmosq_EXPORT int mosquitto_publishv_v5(
		struct mosquitto *mosq,
		int *mid,
		const char *topic,
