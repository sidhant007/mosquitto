...
int mosquitto_publishv_v5(struct mosquitto *mosq, int *mid, const char *topic, const struct vec *buffers, int buffers_cnt, int qos, bool retain, const mosquitto_property *properties);

int mosquitto_publishv(struct mosquitto *mosq, int *mid, const char *topic, const struct vec *buffers, int buffers_cnt, int qos, bool retain)
{
  return mosquitto_publishv_v5(mosq, mid, topic, buffers, buffers_cnt, qos, retain, NULL);
}
...
{
  const struct vec buffer = {payload, (uint32_t) payloadlen};
  return mosquitto_publishv_v5(mosq, mid, topic, &buffer, 1, qos, retain, properties);
}
...
int mosquitto_publishv_v5(struct mosquitto *mosq, int *mid, const char *topic, const struct vec *buffers, int buffers_cnt, int qos, bool retain, const mosquitto_property *properties)
{
	...
	if(qos == 0){
		return send__publishv(mosq, local_mid, topic, payloadlen, buffers, buffers_cnt, qos, retain, false, outgoing_properties, NULL, 0);
	}else{
