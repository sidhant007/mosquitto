...
int send__publishm(struct mosquitto *mosq, uint16_t mid, const char *topic, uint32_t payloadlen, const void **payload, uint32_t *lengths, int cnt, int qos, bool retain, bool dup, const mosquitto_property *cmsg_props, const mosquitto_property *store_props, uint32_t expiry_interval)
{
#ifdef WITH_BROKER
	size_t len;
#ifdef WITH_BRIDGE
		...
		for(i=0; i<mosq->bridge->topic_count; i++){
			...
			if((cur_topic->direction == bd_both || cur_topic->direction == bd_out)
				...
				if(match){
					...
					rc =  send__real_publishm(mosq, mid, mapped_topic, payloadlen, payload, lengths, cnt, qos, retain, dup, cmsg_props, store_props, expiry_interval);
					mosquitto__free(mapped_topic);
					return rc;
				}
			}
	...
	return send__real_publishm(mosq, mid, topic, payloadlen, payload, lengths, cnt, qos, retain, dup, cmsg_props, store_props, expiry_interval);
}
...
int send__real_publishm(struct mosquitto *mosq, uint16_t mid, const char *topic, uint32_t payloadlen, const void **payload, uint32_t *lengths, int cnt, int qos, bool retain, bool dup, const mosquitto_property *cmsg_props, const mosquitto_property *store_props, uint32_t expiry_interval)
{
	...
	/* Payload */
	if(payloadlen){
		packet__write_multiple(packet, payload, lengths, cnt);
	}
